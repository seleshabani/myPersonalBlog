...
indexRouter.get('/','home',async (req,res)=>{
    try {
        let topView = await postModel.find().populate('categorie').populate('sousCategorie').
        sort({view:'desc'}).limit(4);
        let latestPost = await postModel.find().populate('categorie').populate('sousCategorie').sort({createdAt:'desc'}).limit(3);
...
indexRouter.get('/contact','contact',(req,res)=>{
